...
import { BIconTrash2Fill, BIconXCircleFill } from 'bootstrap-icons-vue';
...
export default {
	...
	setup() {
		...
		const hexToBase64 = (string) =>
			Buffer.from(string, 'hex').toString('base64');
		...
		const secretContentHTML = computed(() =>
			secretContent.value.replace(/\n/g, '<br>'),
		);
		...
		const getItemData = async () => {
			...
			try {
				...
				if (res.status === 200 && res.data) {
					...
					secretCreationDate.value = moment(item.creation_date).format(
						'YYYY-MM-DD HH:mm:ss',
					);
		...
		const deleteItemFromCloud = async () => {
			...
			const res = await axios.delete(
				`${import.meta.env.VITE_API_URL}/secret/delete/${accessKeyHash2}/${manageKeyHash2}`,
				{
					data: { confirm: true },
				},
			);
		...
		const decryptSecret = () => {
			...
			try {
				...
				const decryptedTest = CryptoJS.AES.decrypt(
					testBase64,
		...
		watch(isReady, (newVal, oldVal) => {
			...
			if (
				newVal === true &&
				secretItem.value.content !== undefined &&
				isProtected.value === false
			) {
				// (isProtected.value === false || isOwner.value === true)
				decryptSecret();
			}
		});
